...
export default function OrdersPage() {
  ...
  }, [isLoggedIn, user?.email, showModal]); // user.email을 의존성 배열에 추가
  ...
  const statusCounts = useMemo(() => {
    ...
    allOrders.forEach(order => {
      ...
      if (status === 'order' || status === 'paypal' || status === 'pay in cash') {
        counts.Payment++; // 'Order' 또는 'Payment' 관련 상태를 Payment로 묶음
      } else if (status === 'ems') {
        counts.Order++; // EMS는 'Order'로 분류
  ...
  const displayedOrders = useMemo(() => {
    ...
    filtered = filtered.filter(order => {
      ...
      if (activeStatusTab === 'Order') {
        ...
        return status === 'order' || status === 'paypal' || status === 'pay in cash';
    ...
    if (activeSubFilter !== 'All') {
      filtered = filtered.filter(order => {
        const status = order.status?.toLowerCase();
        const subFilterLower = activeSubFilter.toLowerCase().replace(/ /g, ''); // 공백 제거
  ...
  const subFilterOptions = useMemo(() => {
    if (activeStatusTab === 'Order') {
      return ['All', 'EMS']; // EMS만 있다고 가정
    } else if (activeStatusTab === 'Payment') {
      return ['All', 'Order', 'Paypal', 'Pay in Cash'];
    } else if (activeStatusTab === 'Delivered') {
  ...
  const handleMoreClick = async (orderId, type) => {
    if (type === 'Order') {
      // 주문 확인 상세 페이지 이동 (어드민 페이지로 리다이렉트)
      router.push(`/admin/order-management/${orderId}`);
            ...
            displayedOrders.map(order => (
              <div key={order.orderId} className={styles.orderItem}>
                <div className={styles.orderHeader}>
                  ...
                  <span className={`${styles.orderStatusBadge} ${styles[order.status?.toLowerCase().replace(/ /g, '')]}`}>
                    {order.status}
                  </span>
                </div>
                <div className={styles.orderDetails}>
                  <p><strong>Total:</strong> ${order.totalAmount?.toFixed(2) || '0.00'}</p>
                  <p><strong>Date:</strong> {moment(order.date).format('YYYY.MM.DD')}</p>
                  <p><strong>Customer:</strong> {order.userName}</p>
                  <p><strong>Ship:</strong> {order.shipName}</p>
                </div>
                <button
