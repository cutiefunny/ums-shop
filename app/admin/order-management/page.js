...
// 읽지 않은 사용자 메시지가 있는지 확인하는 헬퍼 함수
const hasUnreadUserMessage = (messages) => {
    if (!messages || !Array.isArray(messages)) {
        return false;
    }
    // sender가 'user'이고 isRead가 false이거나 isRead가 없으면 읽지 않은 메시지로 간주
    console.log('Checking for unread user messages:', messages);
    return messages.some(msg => msg.sender === 'User' && (!msg.isRead || msg.isRead === undefined));
};
...
export default function OrderManagementPage() {
    ...
    const renderPagination = () => {
        ...
        // If there are no pages (e.g. totalPages === 0), still show page 1
        if (pages.length === 0) {
            pages.push(
                <button
                    key={1}
                        ...
                        currentOrders.map(order => (
                            <tr key={order.orderId}>
                                <td className={styles.orderIdCell}>
                                    ...
                                    {hasUnreadUserMessage(order.messages) && (
                                        <img 
                                            src="/images/new-message-badge.png" 
                                            alt="New Message" 
