...
// 배송 상태 데이터를 가져올 테이블 (delivery-status)
const TABLE_DELIVERY_STATUS = process.env.DYNAMODB_TABLE_DELIVERY_STATUS || 'ums-shop-delivery-status';
...
export async function PATCH(request) {
  try {
    const { orderId, status, trackingNumber } = await request.json(); // trackingNumber 추가

    if (!orderId || !status) {
      return NextResponse.json({ message: 'Order ID and status are required' }, { status: 400 });
    }

    let updateExpression = 'SET #status = :statusVal';
    ...
    // trackingNumber가 제공된 경우 업데이트 표현식에 추가
    if (trackingNumber !== undefined && trackingNumber !== null) { // 빈 문자열도 유효한 값으로 처리
      updateExpression += ', #trackingNumber = :trackingNumberVal';
      expressionAttributeNames['#trackingNumber'] = 'trackingNumber';
      expressionAttributeValues[':trackingNumberVal'] = trackingNumber;
    }
